...
def init_game():
    global items, wall_coords, bad_turtles , x, y
    items = {}
    wall_coords = []
    item_coords = []
    ...
    for (i,j) in (apple_coords):
        x = random.randint(-1 * width/2 + 8, width/2 - 8)       # rastegele x koordinati belirlenir.
        y = random.randint(-1 * height/2 + 8, height/2 - 8)
...
"""def pause():
   mario['turtle'].speed = 0
   bad_turtle['turtle'].speed = 0"""
